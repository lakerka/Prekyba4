...
public class TopModel extends AbstractTableModel {
     ...
     * 
     */
    private static final long serialVersionUID = 5388619581794909198L;

    public TopModel(TopDataResultRowsHandler topDataResultRowsHandler) {

        if (topDataResultRowsHandler == null) {
    ...
    public void addRow(NewContract contract) {
        ...
        m_rows.add(row);
        topDataResultRowsHandler.reqTopMktData(contract, "", false, row);
    ...
    public void desubscribe() {
        for (TopRow row : m_rows) {
            topDataResultRowsHandler.cancelTopMktData(row);
        }
    }
    ...
    public void cancel(int i) {
        topDataResultRowsHandler.cancelTopMktData(m_rows.get(i));
    }

    static class TopRow implements ITopMktDataHandler {
        ...
        @Override
        public void tickPrice(NewTickType tickType, double price,
                int canAutoExecute) {
            switch (tickType) {
            case BID:
                m_bid = price;
